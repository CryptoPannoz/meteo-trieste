(function () {
  ...
  window.stationChartHtml = function (rows, color, lang) {
    ...
    var levels=[yMax,yMax/2,0].map(function(v){return '<line x1="'+sx+'" y1="'+y(v)+'" x2="'+(w-dx)+'" y2="'+y(v)+'" stroke="#35404a" stroke-width="1" stroke-dasharray="4 5"/><text x="'+(sx-7)+'" y="'+(y(v)+4)+'" text-anchor="end" font-size="11" fill="#98a4ad">'+v+'</text>';}).join("");
    var axis=data.map(function(r,i){var deg=degrees[String(r.dir).toUpperCase().trim()];var arrow=deg==null?"":'<g transform="rotate('+deg+')"><path d="M0 -8V7M-4 3L0 7L4 3" fill="none" stroke="#f0f43c" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></g>';return '<g transform="translate('+x(i)+',176)">'+arrow+'<text x="0" y="18" text-anchor="middle" font-size="10" font-weight="700" fill="#f4f7f8">'+esc(r.dir)+'</text><text x="0" y="36" text-anchor="middle" font-size="11" fill="#98a4ad">'+esc(r.time)+'</text></g>';}).join("");
    ...
    return '<div class="wind-chart"><div class="chart-tip" role="status"></div><svg viewBox="0 0 '+w+' '+h+'" role="img"><defs><linearGradient id="'+id+'" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="'+color+'" stop-opacity=".24"/><stop offset="1" stop-color="'+color+'" stop-opacity=".03"/></linearGradient></defs>'+levels+'<text x="8" y="13" font-size="10" fill="#98a4ad">kt</text><polygon fill="url(#'+id+')" points="'+area+'"/><polyline fill="none" stroke="#74808a" stroke-width="3.6" stroke-dasharray="7 5" stroke-linejoin="round" stroke-linecap="round" points="'+points("gust")+'"/><polyline fill="none" stroke="'+color+'" stroke-width="5.2" stroke-linejoin="round" stroke-linecap="round" points="'+points("kt")+'"/>'+axis+'<g class="chart-cursore" visibility="hidden"><line y1="'+sy+'" y2="'+base+'" stroke="#f0f43c" opacity=".45"/><circle r="6" fill="#0b1014" stroke="'+color+'" stroke-width="3.5"/></g>'+hits+'</svg><div class="chart-legend"><span class="key" style="--c:'+color+'">'+labels.wind+'</span><span class="key" style="--c:#74808a">'+labels.gust+'</span><span>'+labels.direction+'</span></div></div>';
  };
